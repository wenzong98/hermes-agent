...
class FakeLocalMemory:
    ...
    def search(self, **kwargs):
        return {"results": [{"memory": "local search hit", "score": 0.9}]}

    def get_all(self, **kwargs):
        return {"results": [{"memory": "local profile hit"}]}
...
class FakeCollection:
    ...
    def get(self, *, where=None, include=None):
        ...
        return {"metadatas": [{"data": f"local profile hit {i}", "user_id": "tg_user_1"} for i in range(25)]}
    ...
    def query(self, *, query_embeddings, n_results, where=None, include=None):
        ...
        return {
            "metadatas": [[
                {"data": "local search hit", "user_id": "tg_user_1"},
            ]],
            "distances": [[0.1]],
        }
...
def test_local_mode_tool_calls_go_through_local_memory(monkeypatch, tmp_path):
    ...
    assert "local profile hit 24" in profile["result"]
    assert search["count"] == 1
    assert search["results"][0]["memory"] == "local search hit"
    assert conclude["result"] == "Fact stored."

    assert fake_local.vector_store.collection.get_calls[0]["where"] == {"user_id": "tg_user_1"}
    assert fake_local.vector_store.collection.get_calls[0]["include"] == ["metadatas"]
    assert fake_local.vector_store.collection.query_calls[0]["where"] == {"user_id": "tg_user_1"}
    assert fake_local.vector_store.collection.query_calls[0]["include"] == ["metadatas", "distances"]
